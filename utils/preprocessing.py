...
def _aggregate_rnn_autoencoder_data(coordinates, input_length, input_gap=0, pred_length=0,
                                    return_start_idxs=False):  # added return_start_idxs
...
def aggregate_rnn_autoencoder_data(trajectories, input_length, input_gap=0, pred_length=0,
                                   return_ids=False):  # added return_ids
    Xs, Xs_pred = [], []
    ...
    for trajectory in trajectories.values():
        ...
        if return_ids:  # added
            X, X_pred, input_ranges_idxs, future_ranges_idxs = _aggregate_rnn_autoencoder_data(trajectory.coordinates,
                                                                                               input_length,
                                                                                               input_gap, pred_length,
                                                                                               return_start_idxs=return_ids)  # added last two return values
            ...
            input_trajectory_ids += [[scene_id, clip_id, int(trajectory.person_id), frames[input_start_idx]] for
                                     input_start_idx in input_start_idxs]  # added
            input_segs_ids += [frames[range_] for range_ in input_ranges_idxs]  # added

        else:  # added
            X, X_pred = _aggregate_rnn_autoencoder_data(trajectory.coordinates, input_length, input_gap,
                                                        pred_length)  # original line of code
        ...
        if X_pred is not None:
            ...
            if return_ids:  # added
                ...
                future_trajectory_ids += [[scene_id, clip_id, int(trajectory.person_id), frames[future_start_idx]] for
                                          future_start_idx in future_start_idxs]  # added
